...
`gmod-api-db` 是 Lux 对 GMod 生态提供高质量开发体验的基础。它不是简单的补全列表，也不是人工维护的 API 表，而是由官方文档生成的完整文档数据库，加上供编译器和 LSP 共用的 API 语义索引。
...
Lux 使用 `https://wiki.facepunch.com/gmod/~pagelist?format=json` 作为覆盖率基准，然后抓取每一个官方页面的 `?format=json` 数据，解析其中的 Facepunch markup。official pagelist 中的每一个页面都必须生成一个 `documents[]` record；API 页面还会额外生成结构化的 `entries[]`、`hooks[]` 和 `classes[]` record。
...
手写数据只能作为测试 fixture 或 override patch，不能作为主数据库来源。override 必须可追溯，且不能替代、缩减或绕过官方抓取链路。
...
- coverage manifest 必须能说明官方页总数、document page 数量、API 候选页数量、结构化解析数量、fallback 文档页数量、跳过的非 API 页面和失败页面。

当前 bundled database 为全部 6335 个官方页面生成了 document record，并为其中 6121 个 API 候选页面生成语义 API 索引；6121 个页面结构化解析，0 个页面作为 fallback 文档页保留，生成 10022 个 entry、497 个 hook、186 个 class，失败转换为 0。
  ...
  "entries": [
    {
      ...
      "kind": "function",
      "realm": "shared",
      "summary": "经过审查后的修正文档摘要。"
    }
  ]
}
```

entry 按 `path` 合并，hook 按 `name` 合并，class 按 `name` 合并。

## 数据模型

数据库分为两层：

- `documents[]`：Facepunch 官方页表中每个页面一个 record，包括教程、说明页和非 API reference 页。这是官方文档完整覆盖层。
- `entries[]`、`hooks[]`、`classes[]`：从 API markup 中抽取的结构化 API 符号，用于补全、realm 检查、signature help 和方法查询。
